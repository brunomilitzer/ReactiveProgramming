...
public class QuoteGeneratorServiceImpl implements QuoteGeneratorService {
    ...
    @Override
    public Flux<Quote> fetchQuoteStream( final Duration period ) {
        ...
        return Flux.generate( () -> 0,
                ( BiFunction<Integer, SynchronousSink<Quote>, Integer> ) ( index, sink ) -> {
                    final Quote updatedQuote = this.updatedQuote( this.quotes.get( index ) );
                ...
                .log( "com.brunomilitzer.service.QuoteGeneratorService" );
    }

    private Quote updatedQuote( final Quote quote ) {

        final BigDecimal priceChange = quote.getPrice()
